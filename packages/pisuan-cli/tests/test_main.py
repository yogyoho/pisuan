...
def test_chat_command_starts_web_chat(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("pisuan_cli.main._store", lambda: store)
    monkeypatch.setattr("pisuan_cli.main.run_web_chat", fake_run_web_chat)
...
def test_remote_command_prints_version_and_remote_context_first(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("pisuan_cli.main._store", lambda: store)
    monkeypatch.setattr("pisuan_cli.main.remote_ping", fake_remote_ping)
    ...
    assert lines[:3] == [f"Pisuan CLI {__version__}", "Remote: local https://example.com", "pong"]
