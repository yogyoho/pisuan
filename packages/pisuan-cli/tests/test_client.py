...
from pisuan_cli.client import ClientError, PisuanClient, _iter_sse_events
from pisuan_cli.config import Remote


def _patched_client(monkeypatch):
    client = PisuanClient(Remote(name="local", url="http://localhost:5173", api_key="yxkey_test"))
    calls: list[dict] = []
