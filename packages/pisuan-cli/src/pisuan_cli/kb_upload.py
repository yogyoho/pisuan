...
def add_uploaded_documents(client: PisuanClient, kb_id: str, uploaded: list[UploadResult]) -> dict:
...
def _ensure_kb_upload_supported(client: PisuanClient) -> None:
    try:
...
def _load_kb_types(client: PisuanClient) -> dict:
...
def _resolve_database(client: PisuanClient, kb_id: str | None, kb_types: dict, console: Console) -> dict:
...
def _list_uploadable_databases(client: PisuanClient, kb_types: dict) -> list[dict]:
...
def _load_supported_extensions(client: PisuanClient) -> set[str]:
