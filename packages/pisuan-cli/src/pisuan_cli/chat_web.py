...
from pisuan_cli.config import ConfigStore
...
class ChatWebServer(ThreadingHTTPServer):
    """仅监听本机并代理 Pisuan Agent 请求的临时 HTTP 服务。"""
...
class ChatRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        template = files("pisuan_cli").joinpath("chat.html").read_text(encoding="utf-8")
    ...
    def do_POST(self) -> None:
        ...
        if not secrets.compare_digest(
            self.headers.get("X-Pisuan-Chat-Token", ""), self.server.session_token
        ):
...
def run_web_chat(
    ...
    *,
    ...
    if not remote.has_api_key:
        raise ChatWebError("当前 remote 尚未登录，请先运行 pisuan login")

    client = PisuanClient(remote)
    server = ChatWebServer(
