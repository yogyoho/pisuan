...
from pisuan_cli import __version__
from pisuan_cli.agent import AgentError, run_agent_list, run_agent_show
from pisuan_cli.agent_eval import AgentEvalError, AgentEvalOptions, run_langfuse_agent_experiment
from pisuan_cli.chat_web import ChatWebError, run_web_chat
from pisuan_cli.client import ClientError
from pisuan_cli.commands import (
    CommandError,
    login_with_api_key,
    login_with_browser,
...
from pisuan_cli.config import ConfigError, ConfigStore
from pisuan_cli.kb import (
    KbError,
    run_kb_files,
    run_kb_find,
    run_kb_list,
...
from pisuan_cli.kb_upload import DEFAULT_CONCURRENCY, MAX_CONCURRENCY, KbUploadError, KbUploadOptions, run_kb_upload

console = Console()
app = typer.Typer(help="Pisuan command line client.", invoke_without_command=True)
remote_app = typer.Typer(help="Manage Pisuan remotes.")
agent_app = typer.Typer(help="Run and manage Pisuan agents.")
...
def _print_remote_context(store: ConfigStore, remote_name: str | None) -> None:
    ...
    console.print(f"Pisuan CLI {__version__}")
...
def chat(
    ...
    agent_slug: str = typer.Option(
        "default-chatbot", "--agent-slug", help="Pisuan agent slug."
    ),
...
def eval_agent(
    ...
    agent_slug: str = typer.Option(..., "--agent-slug", help="Pisuan agent slug."),
    ...
    timeout_seconds: float = typer.Option(900, "--timeout-seconds", help="Per item Pisuan API timeout."),
):
...
def show_agent(
    agent_slug: str = typer.Argument(..., help="Pisuan agent slug."),
    remote: str | None = typer.Option(None, "--remote", help="Remote name."),
    as_json: bool = typer.Option(False, "--json", help="Output raw JSON."),
):
