...
JWT_AUDIENCE = "pisuan-know-api"
PUBLIC_DEFAULT_JWT_SECRET_KEY = "pisuan_know_secure_key"
...
def _is_production_env() -> bool:
    return os.environ.get("PISUAN_ENV", "development").strip().lower() in {"prod", "production"}
...
def _get_jwt_issuer() -> str:
    instance_id = _get_or_create_dev_env("PISUAN_INSTANCE_ID", lambda: f"instance-{secrets.token_hex(8)}")
    return f"pisuan-know:{instance_id}"
...
class AuthUtils:
    ...
    def derive_api_key(idempotency_scope: str, subject_id: int | str) -> tuple[str, str, str]:
        ...
        payload = f"pisuan-api-key-v1:{subject_id}:{scope}".encode()
