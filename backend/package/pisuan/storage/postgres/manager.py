...
from pisuan.storage.postgres.models_business import Base as BusinessBase
from pisuan.storage.postgres.models_knowledge import Base as KnowledgeBase
from pisuan.storage.postgres.models_domain_factory import (
    Base as DomainFactoryBase,
)
from pisuan.storage.postgres.models_domain_entity import (
    Base as DomainEntityBase,
)
from pisuan.utils import logger
from pisuan.utils.singleton import SingletonMeta

AGENT_RUN_TERMINAL_STATUS_SQL = ", ".join(f"'{status}'" for status in AGENT_RUN_TERMINAL_STATUSES)
BUSINESS_SCHEMA_VERSION = 8
KNOWLEDGE_SCHEMA_VERSION = 2
SCHEMA_VERSION_TABLE = "pisuan_schema_migrations"
...
class PostgresManager(metaclass=SingletonMeta):
    ...
    async def schema_migration_lock(self):
        ...
        async with self.async_engine.connect() as conn:
            params = {"lock_scope": "pisuan:schema-migration"}
            ...
            try:
                ...
                if unlocked is not True:
                    await conn.close()
                    raise RuntimeError("Failed to release Pisuan schema migration advisory lock")
