...
from pisuan.agents.backends.paths import runtime_user_data_path
...
from pisuan.utils.upload_utils import MAX_UPLOAD_SIZE_BYTES, write_upload_to_path
...
async def download_workspace_file(*, path: str, current_user: User) -> FileResponse:
    ...
    descriptor, temp_path = tempfile.mkstemp(prefix="pisuan-workspace-download-", suffix=PurePosixPath(file_name).suffix)
    os.close(descriptor)
    try:
        await asyncio.to_thread(
...
async def _write_workspace_upload(file: UploadFile, backend: Workspace, target: str) -> dict:
    descriptor, temp_path = tempfile.mkstemp(prefix="pisuan-workspace-upload-")
    os.close(descriptor)
    try:
