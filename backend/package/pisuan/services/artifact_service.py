...
from pisuan.agents.skills.service import ResolvedSkill, list_accessible_skills
...
from pisuan.services.workdir_service import resolve_authorized_workdir
...
async def resolve_thread_artifact_view(
    *,
    ...
    descriptor, temp_path = tempfile.mkstemp(prefix="pisuan-artifact-", suffix=PurePosixPath(normalized).suffix)
    os.close(descriptor)
    try:
...
async def save_thread_artifact_to_workspace_view(
    *, thread_id: str, current_uid: str, db, path: str, destination_path: str | None = None
    ...
    skill_source = await _require_skill_artifact_access(normalized_path=normalized, current_uid=current_uid, db=db)
    descriptor, temp_path = tempfile.mkstemp(prefix="pisuan-save-artifact-")
    os.close(descriptor)
    try:
