...
from pisuan.agents.backends.paths import runtime_path_for_workdir_scope, workdir_scope_from_runtime_path
...
from pisuan.storage.minio import StorageError, get_minio_client
from pisuan.utils.datetime_utils import utc_isoformat
from pisuan.utils.logging_config import logger
from pisuan.utils.upload_utils import read_upload_with_limit
...
async def _write_workdir_file(workdir, path: str, content: bytes) -> None:
    ...
    temp_path = ""
    try:
        with tempfile.NamedTemporaryFile(prefix="pisuan-attachment-", delete=False) as temp_file:
...
async def confirm_tmp_thread_attachments_view(
    *,
    ...
    from pisuan.services.workdir_service import resolve_authorized_conversation_workdir
