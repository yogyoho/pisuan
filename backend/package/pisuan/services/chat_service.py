...
from pisuan.agents.backends.paths import runtime_workdir_path
from pisuan.agents.base import _json_safe
from pisuan.agents.buildin import get_agent_backend
...
from pisuan.agents.state import AgentStatePayload
...
from pisuan.repositories.subagent_thread_repository import SubagentThreadRepository
...
from pisuan.services.attachment_service import serialize_attachment
from pisuan.services.input_message_service import AgentRunInputMessage
from pisuan.services.langfuse_service import (
    LangfuseRunContext,
    build_run_context,
    flush_langfuse,
    get_trace_info,
)
from pisuan.services.model_message_audit_service import ModelMessageAuditCollector
from pisuan.services.run_queue_service import publish_cancel_signals
from pisuan.services.subagent_run_service import serialize_subagent_run_state
from pisuan.services.tool_message_audit_service import ToolMessageAuditCollector
from pisuan.services.workdir_service import resolve_conversation_workdir_path
from pisuan.storage.postgres.manager import pg_manager
from pisuan.storage.postgres.models_business import MODEL_AUDIT_MESSAGE_TYPE, Agent, Conversation, User
from pisuan.utils.datetime_utils import utc_now_naive
from pisuan.utils.logging_config import logger
from pisuan.utils.question_utils import (
    normalize_questions as _normalize_interrupt_questions,
)
from pisuan.utils.thread_utils import extract_thread_id as _metadata_thread_id
...
def _message_chunk_pisuan_events(
...
def _protocol_event_pisuan_event(
    event: dict[str, Any],
    *,
    message_id: str | None,
    thread_id: str | None,
...
def _context_compression_payload(payload: Any) -> dict | None:
    if isinstance(payload, dict) and payload.get("type") == "pisuan.context_compression":
        return payload
    return None
...
def _message_payload_pisuan_events(
    msg: Any,
    *,
    ...
    if isinstance(msg, dict) and isinstance(msg.get("event"), str):
        ...
        stream_event = _protocol_event_pisuan_event(
            msg,
            message_id=message_id,
            thread_id=thread_id,
            namespace=namespace,
    ...
    return _message_chunk_pisuan_events(
        msg_dict,
        message_id=message_id,
        thread_id=thread_id,
        namespace=namespace,
    ...
    try:
        ...
        async with aclosing(stream_source):
            async for mode, payload in stream_source:
                ...
                stream_events = _message_payload_pisuan_events(
                    msg,
                    metadata=metadata,
                    namespace=namespace,
