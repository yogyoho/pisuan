...
from pisuan.agents.backends.sandbox.provider import get_sandbox_provider
...
from pisuan.repositories.agent_run_repository import TERMINAL_RUN_STATUSES, AgentRunRepository
...
from pisuan.services.chat_service import get_agent_state_view, stream_agent_chat, stream_agent_resume
from pisuan.services.input_message_service import restore_chat_input_message
from pisuan.services.run_queue_service import (
...
from pisuan.services.task_queue_service import (
...
from pisuan.services.task_service import TASKER_DEFAULT_TIMEOUT_SECONDS, process_task
from pisuan.services.workdir_service import (
    AuthorizedWorkdir,
    resolve_authorized_workdir,
    resolve_conversation_workdir_path,
)
from pisuan.storage.postgres.manager import pg_manager
from pisuan.storage.postgres.models_business import AgentRun, Conversation, Message, User
...
async def _release_runtime_if_idle(run: AgentRun) -> bool:
    ...
    async with pg_manager.get_async_session_context() as db:
        await db.execute(
            ...
            {"lock_key": f"pisuan-runtime-cleanup:{run.uid}:{runtime_scope_id}"},
        )
...
def _map_chunk_to_run_event(chunk: dict) -> tuple[str, dict]:
    ...
    if status == "agent_state":
        return "custom", {"name": "pisuan.agent_state", "chunk": chunk, "agent_state": chunk.get("agent_state") or {}}
...
async def _worker_shutdown(ctx):
    ...
    from pisuan.services.run_queue_service import close_queue_clients
...
class WorkerSettings:
    ...
    job_timeout = int(os.getenv("PISUAN_JOB_TIMEOUT_SECONDS", "3600"))
    keep_result = 60
