...
from pisuan.agents.backends.paths import is_runtime_path, runtime_path_for_workdir_scope
from pisuan.services.file_preview import render_file_preview
from pisuan.services.workdir_service import AuthorizedWorkdir, resolve_authorized_workdir
from pisuan.utils.datetime_utils import utc_isoformat_from_timestamp
from pisuan.utils.filepreview import (
    MAX_BINARY_PREVIEW_SIZE_BYTES,
    OfficePreviewConversionError,
    preview_too_large,
)
from pisuan.utils.upload_utils import write_upload_to_path
...
async def _download_to_temp(workdir, path: str, max_bytes: int) -> str:
    descriptor, temp_path = tempfile.mkstemp(prefix="pisuan-viewer-", suffix=PurePosixPath(path).suffix)
    os.close(descriptor)
    try:
...
async def upload_viewer_files(*, thread_id: str, parent_path: str, files: list[UploadFile], current_user, db) -> dict:
    ...
    for upload, file_name in zip(files, file_names, strict=True):
        descriptor, temp_path = tempfile.mkstemp(prefix="pisuan-viewer-upload-")
        os.close(descriptor)
        try:
