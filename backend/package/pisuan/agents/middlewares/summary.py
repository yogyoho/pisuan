"""Pisuan 对 DeepAgents 会话摘要中间件的适配。"""
...
_TOOL_RESULT_SAVED_MARKER = "pisuan_tool_result_saved"
...
_SUMMARY_COMPRESSION_STATE: ContextVar[dict[str, bool] | None] = ContextVar(
    "pisuan_summary_compression_state",
    default=None,
)


class PisuanSummarizationMiddleware(SummarizationMiddleware):
...
def create_summary_middleware(
    ...
    *,
    ...
    return PisuanSummarizationMiddleware(**middleware_kwargs)


def create_summary_middleware_from_context(context, *, backend) -> PisuanSummarizationMiddleware:
    ...
    return create_summary_middleware(
        ...
        summary_prompt=getattr(context, "summary_prompt", None) or DEFAULT_PISUAN_SUMMARY_PROMPT,
...
def _emit_compression(status: str, **extra: Any) -> None:
    try:
        ...
        return
    writer({"type": "pisuan.context_compression", "status": status, **extra})
