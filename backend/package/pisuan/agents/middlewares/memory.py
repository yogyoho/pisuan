...
from pisuan.services.memory_service import (
    load_memory_prompt,
...
async def create_memory_middleware(context) -> PisuanMemoryMiddleware | None:
    """仅在用户开启 Memory 时创建主 Agent middleware。"""
    memory_content = await load_memory_prompt(str(getattr(context, "uid", "") or ""))
    if memory_content is None:
        return None
    return PisuanMemoryMiddleware(memory_content)


class PisuanMemoryMiddleware(AgentMiddleware[Any, ContextT, ResponseT]):
