...
from pisuan.agents.backends.paths import (
    VIRTUAL_PATH_PREFIX,
    VIRTUAL_SKILLS_PATH,
)
from pisuan.agents.backends.sandbox import ProvisionerSandboxBackend
from pisuan.agents.toolkits.registry import ToolExtraMetadata, _all_tool_instances, _extra_registry, tool
from pisuan.config.options import system_options
from pisuan.repositories.domain_factory_repository import (
    DomainFactoryRepository,
    _normalize_domain,
    _normalize_report_type,
)
from pisuan.utils import logger
from pisuan.utils.question_utils import normalize_questions
...
async def ocr_parse_file(file_path: str, runtime: ToolRuntime, ocr_engine: str | None = None) -> dict:
    ...
    from pisuan.services.ocr_service import resolve_ocr_engine_id
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile(prefix="pisuan-ocr-source-", suffix=suffix, delete=False) as temp_file:
        ...
        with tempfile.NamedTemporaryFile(prefix="pisuan-ocr-output-", delete=False) as temp_file:
...
async def assemble_report(report_id: str, runtime: ToolRuntime) -> dict:
    ...
    """
    from pisuan.services.ref_resolver import _MISSING_RE, resolve_refs
...
async def lookup_standard_indicator(pollutant: str, doc_code: str | None = None) -> dict:
    ...
    try:
        from pisuan.extensions.regulation_library.enrichment_service import query_indicators
    except ImportError:
