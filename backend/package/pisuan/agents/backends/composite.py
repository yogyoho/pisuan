...
from pisuan.agents.skills.service import refresh_user_skill_projection_async

from .sandbox import ProvisionerSandboxBackend

# Pisuan 在 DeepAgents 内建排除集之上额外豁免知识库文档工具结果，
...
# 文件工具 allowlist：显式排除 destructive delete。Pisuan backend 未实现 delete，
...
class PisuanFilesystemMiddleware(FilesystemMiddleware):
...
class _BackendScope:
    ...
    def create_backend(self) -> CompositeBackend:
        ...
        # 派生 large_tool_results 与 conversation_history 前缀，与 Pisuan 契约一致。
...
def create_agent_filesystem_middleware(
    ...
    *,
    ...
    return PisuanFilesystemMiddleware(
        backend=backend,
