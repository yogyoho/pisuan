...
class ProvisionerSandboxBackend(BaseSandbox):
    ...
    def _read_file_base64(self, path: str) -> str:
        path_b64 = base64.b64encode(path.encode("utf-8")).decode("ascii")
        output_path = f"/tmp/pisuan-read-file-{uuid.uuid4().hex}.b64"
...
"""
    ...
    def upload_authorized_file_from_path(self, path: str, source_path: str) -> None:
        ...
        export_path = f"/tmp/.pisuan-file-upload-{uuid.uuid4().hex}"
...
temp_name = {f".pisuan-write-{uuid.uuid4().hex}"!r}
directory_fd = None
source_fd = None
target_fd = None
try:
...
try:
    ...
    print("PISUAN_SAFE_LIST " + base64.b64encode(json.dumps(entries).encode()).decode())
finally:
    os.close(directory_fd)
"""
        encoded_script = base64.b64encode(script.encode("utf-8")).decode("ascii")
        ...
        payload = next(
            (
                line.removeprefix("PISUAN_SAFE_LIST ")
                for line in (result.output or "").splitlines()
                if line.startswith("PISUAN_SAFE_LIST ")
            ),
            None,
        )
        if payload is None:
...
"""
    ...
    def _download_scoped_file_to_path(self, normalized_path: str, root: str, target_path: str, max_bytes: int) -> int:
        ...
        export_path = f"/tmp/.pisuan-file-snapshot-{uuid.uuid4().hex}"
        script = f"""
import hashlib
import os
import stat
...
try:
    ...
    os.close(target_fd)
    target_fd = None
    print(f"PISUAN_FILE_SNAPSHOT {{size}} {{digest.hexdigest()}}")
except Exception:
    if target_fd is not None:
        os.close(target_fd)
    try:
...
"""
        ...
        try:
            ...
            snapshot_line = next(
                (line for line in str(result.output or "").splitlines() if line.startswith("PISUAN_FILE_SNAPSHOT ")),
                None,
            )
            if snapshot_line is None:
                ...
                snapshot_line = next(
                    (
                        ...
                        if line.startswith("PISUAN_FILE_SNAPSHOT ")
                    ),
                    None,
                )
                if snapshot_line is None:
