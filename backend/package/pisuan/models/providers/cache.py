...
from pisuan.storage.redis import sync_redis_client
from pisuan.utils.logging_config import logger

REDIS_CACHE_KEY = "pisuan:model_cache"
_CACHE_TTL_SECONDS = 5
# UID 签名密钥只从这一个固定环境变量读取：provider 配置不接收环境变量名，
# 从结构上消除借签名头探测或离线猜解服务器其他环境变量的通道。
USER_UID_SIGNATURE_SECRET_ENV = "PISUAN_UID_SIGNATURE_SECRET"
...
class ModelInfo:
    ...
    # 开启后聊天模型请求注入带 HMAC 签名的 x-pisuan-uid 头，供外部网关按用户计量。
...
class ModelCache:
    ...
    def rebuild(self, providers: list[Any]) -> None:
        from pisuan.models.providers.service import resolve_api_key
