...
from pisuan.models.providers.cache import USER_UID_SIGNATURE_SECRET_ENV
from pisuan.models.providers.repository import (
...
from pisuan.storage.postgres.models_business import ModelProvider
...
def _validate_user_uid_header(provider_type: str, include_user_uid: bool) -> None:
    """校验 UID 头适配能力与签名密钥，避免保存不生效的 Gemini 设置。"""
    if not include_user_uid:
        return
    if provider_type == "gemini":
        raise ValueError("Gemini 供应商暂不支持请求携带用户 ID")
    if not (os.getenv(USER_UID_SIGNATURE_SECRET_ENV) or "").strip():
        raise ValueError(
...
async def update_provider_config(
    ...
    _validate_user_uid_header(
        payload.get("provider_type", provider.provider_type),
        payload.get("include_user_uid", provider.include_user_uid),
    )
    payload["updated_by"] = username
...
async def test_model_status_by_spec(spec: str) -> dict:
    ...
    try:
        if info.model_type == "embedding":
            from pisuan.models.embed import select_embedding_model
        ...
        if info.model_type == "rerank":
            from pisuan.models.rerank import get_reranker
