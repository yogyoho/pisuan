...
from pisuan.models.providers.cache import USER_UID_SIGNATURE_SECRET_ENV, ModelInfo, model_cache
from pisuan.utils import get_docker_safe_url, logger

USER_UID_HEADER = "x-pisuan-uid"
USER_UID_TIMESTAMP_HEADER = "x-pisuan-uid-ts"
USER_UID_SIGNATURE_HEADER = "x-pisuan-uid-sig"
    ...
    **kwargs,
    ...
    if info.provider_id in {"opencode", "opencode-go"}:
        kwargs["default_headers"] = {
            **(kwargs.get("default_headers") or {}),
            "User-Agent": f"pisuan/{get_version()}",
    ...
    kwargs["metadata"] = {
        **(kwargs.pop("metadata", None) or {}),
        "pisuan_provider_id": info.provider_id,
        "pisuan_provider_type": info.provider_type,
        "pisuan_model_id": info.model_id,
        "pisuan_model_spec": info.spec,
    }
