...
def test_runtime_directory_default_does_not_fall_back_to_save_dir(monkeypatch, tmp_path: Path):
    ...
    monkeypatch.setenv("PISUAN_LEGACY_STORAGE_DIR", str(save_dir))
    monkeypatch.delenv("PISUAN_RUNTIME_DIR", raising=False)

    runtime_dir = get_runtime_dir()

    assert runtime_dir != get_legacy_storage_dir()
    assert save_dir not in runtime_dir.parents
    assert runtime_dir.name.startswith("pisuan-runtime-")
...
def test_skill_storage_directories_support_explicit_domain_mounts(monkeypatch, tmp_path: Path):
    ...
    monkeypatch.setenv("PISUAN_SKILL_DATA_DIR", str(data_dir))
    monkeypatch.setenv("PISUAN_SKILL_PROJECTION_DIR", str(projection_dir))
...
def test_storage_defaults_do_not_derive_from_legacy_save_dir(monkeypatch, tmp_path: Path):
    ...
    monkeypatch.setenv("PISUAN_LEGACY_STORAGE_DIR", str(legacy_dir))
    monkeypatch.delenv("PISUAN_USER_DATA_DIR", raising=False)
    monkeypatch.delenv("PISUAN_SKILL_DATA_DIR", raising=False)
    monkeypatch.delenv("PISUAN_SKILL_PROJECTION_DIR", raising=False)
