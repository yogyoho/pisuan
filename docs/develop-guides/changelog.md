...
- 新增 GraphQueryService 图谱直查服务（Phase C）：`yuxi/services/graph_query_service.py` 封装 Cypher 直查 Neo4j 图谱（list_chapter_keys/get_chapter_outline/get_templates/lookup_chapter_order），作为知识图谱单一数据源的读取入口；`list_chapter_keys`/`get_chapter_outline`/`get_templates` 三个 buildin 工具改造为图谱优先 + DB 降级（图谱异常或空时回退 DomainFactoryRepository），工具层不再绕过图谱直查 DB。Neo4j 连接参数走 `os.getenv`（与 `graph_builder.py` 一致），非 `yuxi.config`。
