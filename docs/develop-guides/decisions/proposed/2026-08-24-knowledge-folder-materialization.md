...
目录实体化属于写用例，不属于列表 repository。HTTP 路由只完成请求和权限适配；用例流程进入 `pisuan.services`，目录与文件的查询、精确字段更新和事务锁进入 repository。目录创建与文件归属更新按同一知识库目录树锁串行化；同一父目录下的同名真实文件夹必须幂等复用或明确冲突，不能创建平行重复目录。批量上传继续允许文件级部分成功，但每个成功文件的目录链和文件归属必须原子发布，失败不能留下该文件对应的半条目录关系。
