...
- Kubernetes inventory 使用稳定的 `app=pisuan-sandbox` selector 枚举候选 Pod，再要求合法 `sandbox-id`。新增 `managed-by` 标签不能排除升级前 Pod；generation/precondition 继续保护删除操作。
...
| Kubernetes inventory 能发现 legacy Pod | 新 selector 排除无 `managed-by` 的旧 Pod | Kubernetes provisioner | provisioner unit/spec 与源码检查 | 仅含 `app=pisuan-sandbox`、`sandbox-id` 的 Pod 仍被列出 | Inspected；真实 Kubernetes smoke Not run |
