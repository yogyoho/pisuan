...
`models/chat.py` 统一拥有通用聊天入口、模型加载和 Chat Completions 协议适配，Agent 仅消费模型能力。加载与适配不单独拆文件；`pisuan.agents` 为已公开的 Agent 扩展示例保留模型加载函数的转导出，内部调用直接引用 `pisuan.models.chat`。此目录收拢是行为等价的移动，不改变请求、配置或历史契约。
...
复测模型边界：向 API 容器传入 `PISUAN_REASONING_PROBE_MODELS='provider:model,...'`，按需设置 `PISUAN_REASONING_PROBE_THINKING=1`，运行 `uv run --no-sync --group test pytest test/integration/services/test_provider_reasoning_live.py -s`。完整链路：设置 `PISUAN_REASONING_E2E_MODEL='provider:model'`，配置既有 E2E 账号后运行 `test/e2e/test_provider_reasoning_e2e.py`。未显式选模型时探针跳过，避免日常测试产生计费调用。

最终回归：`COMPOSE_PROJECT_NAME=pisuan docker compose exec -T api uv run --no-sync --group test pytest test/unit -m 'not slow' -k 'not test_finish_run_terminal_loser_does_not_append_end_event'` 为 1922 通过、52 跳过、1 排除、7 子测试通过。排除项在既有验证中停滞；容器标准 uv 依赖同步存在权限错误，使用已安装依赖测试，不宣称未排除的标准全量命令通过。Web `pnpm run test:unit` 275 项通过，`lint:check`、Web build、`git diff --check` 通过。浏览器覆盖浅色桌面、深色 375px 与减少动态效果。
