...
`pisuan-cli` 是 Pisuan 的命令行客户端。它可以保存多个实例地址、登录远程实例、上传和查询知识库文件，并启动一个只在本机监听的临时聊天页面。
...
```bash
uv tool install pisuan-cli
```

只想试用一次时，可以直接运行：

```bash
uvx --from pisuan-cli pisuan --help
```
...
pisuan remote use local
pisuan remote ping
```

CLI 会把配置保存到 `~/.pisuan/config.toml`。它会把实例地址标准化为入口地址，并在请求时使用对应的 `/api` 路径；不要把 URL 直接写成某个具体 API 路径。

同时管理多个实例时，重复执行 `remote add`，再用下面的命令切换：

```bash
pisuan remote list
pisuan remote use production
```
...
```bash
pisuan login --browser --no-open
```

也可以导入已经在 Pisuan 中创建的 API Key：

```bash
pisuan login --api-key yxkey_<your-key>
```
...
```bash
pisuan chat --agent-slug my-agent
pisuan chat --remote production --no-open
```
...
```bash
pisuan agent show default-chatbot
```
...
```bash
pisuan kb upload ./docs
```

指定知识库并控制文件类型和并发数：

```bash
pisuan kb upload ./docs --kb-id <kb-id> --concurrency 4
pisuan kb upload ./docs --include-ext md,html,docx
pisuan kb upload ./docs --exclude-ext pdf,png
```
...
```bash
pisuan kb list
pisuan kb files --kb-id <kb-id>
pisuan kb files --kb-id <kb-id> --query handbook --status indexed
...
pisuan kb find --kb-id <kb-id> --file-id <file-id> --pattern "年假[：:][ ]*\\d+" --regex
```
...
命令使用当前 remote 的登录态调用 Pisuan，并把每条样例的结果写回 Langfuse experiment。CLI 不负责创建数据集；数据集管理和评估边界见[智能体评估](../agents/agent-evaluation.md)。
